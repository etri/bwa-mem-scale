...
static inline void *mp_alloc(mempool_t *mp)
{
	if (mp->i == mp->n_elems) {
		if (++mp->top == mp->max) {
			...
			mp->mem = (uint8_t **) realloc(mp->mem, sizeof(void*) * mp->max);
		}
...
rpnode_t *rope_restore_node(const rope_t *r, FILE *fp, int64_t c[6])
{
	...
	if (is_bottom) {
		for (i = 0; i < n; ++i) {
			...
			q = rle_nptr(p[i].p);
			if (fread(p[i].c, 8, 6, fp) != 6)
				return NULL;
			if (fread(q, 2, 1, fp) != 1)
				return NULL;
			assert(*q >= 0 && *q <= UINT16_MAX);
			if (fread(q + 1, 1, *q, fp) != *q)
				return NULL;
		}
	} else {
		for (i = 0; i < n; ++i)
...
rope_t *rope_restore(FILE *fp)
{
	...
	assert(r != NULL);
	if (fread(&r->max_nodes, 4, 1, fp) != 1)
		return NULL;
	if (fread(&r->block_len, 4, 1, fp) != 1)
		return NULL;
